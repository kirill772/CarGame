...
export class Game {
    ...
    if (kind === "garage") {
      import("./managers/GarageManager.js").then(({ GarageManager }) => {
        try {
          ...
          // Для гаража нужен особый layout — расширяем overlay
          this.ui.showOverlay("Гараж", this.garage.renderHTML(), true);
  ...
  startRun(mode) {
    ...
    this._bldSeed = null; // сбрасываем здания для новой трассы
  ...
  _fireEvent(eventId, value = 1) {
    ...
    if (eventId === "refreshShop") {
      if (this.shop && !this.ui.screenOverlayList.classList.contains("hidden") && this.ui.overlayTitle.textContent === "Магазин") {
        this.ui.showOverlay("Магазин", this.shop.renderHTML());
        this.shop.bind(this.ui.overlayBody);
      }
    }
    ...
    if (eventId === "refreshGarage") {
      if (this.garage && !this.ui.screenOverlayList.classList.contains("hidden") && this.ui.overlayTitle.textContent === "Гараж") {
        this.ui.showOverlay("Гараж", this.garage.renderHTML(), true);
        this.garage.bind(this.ui.overlayBody);
      }
    }
  ...
  _renderRoad(ctx) {
    ...
    const horizY  = CANVAS_H * 0.38; // линия горизонта

    const THEMES = {
      city:     { road: ["rgba(5,8,22,0.97)",  "rgba(8,12,32,0.97)"],  border: "rgba(0,229,255,0.22)",  lane: "rgba(255,43,214,0.14)",
                  bldColor: ["#0a0f20","#0c1228","#080d1c"], bldNeon: ["#00e5ff","#ff2bd6","#a6ff00"],
                  sky: ["#020510","#07091e"], fog: "rgba(4,6,20,0.72)" },
      mountain: { road: ["rgba(14,11,9,0.97)",  "rgba(18,14,11,0.97)"], border: "rgba(200,220,255,0.14)",lane: "rgba(255,255,255,0.10)",
                  bldColor: ["#0d1015","#101420"], bldNeon: ["#a6ff00","#00e5ff"],
                  sky: ["#040614","#0d1130"], fog: "rgba(10,12,20,0.65)" },
      tokyo:    { road: ["rgba(8,5,16,0.97)",  "rgba(12,7,22,0.97)"],  border: "rgba(255,0,180,0.18)",  lane: "rgba(255,0,128,0.12)",
                  bldColor: ["#100520","#180828"], bldNeon: ["#ff2bd6","#00e5ff","#ffd000"],
                  sky: ["#050312","#160628"], fog: "rgba(14,4,28,0.70)" },
      desert:   { road: ["rgba(22,17,10,0.97)", "rgba(26,20,12,0.97)"], border: "rgba(255,170,0,0.18)",  lane: "rgba(255,165,0,0.12)",
                  bldColor: ["#1a1008","#22150a"], bldNeon: ["#ffd000","#ff6b00"],
                  sky: ["#120608","#2a0e0a"], fog: "rgba(30,14,6,0.58)" },
      highway:  { road: ["rgba(5,5,12,0.97)",  "rgba(8,8,18,0.97)"],   border: "rgba(0,220,255,0.16)",  lane: "rgba(0,255,255,0.10)",
                  bldColor: ["#050812","#080d1a"], bldNeon: ["#00e5ff","#a6ff00"],
                  sky: ["#020308","#08091a"], fog: "rgba(2,3,14,0.75)" },
      megacity: { road: ["rgba(4,8,14,0.97)",  "rgba(6,11,18,0.97)"],  border: "rgba(0,255,128,0.16)",  lane: "rgba(0,255,128,0.11)",
                  bldColor: ["#030a10","#040d18","#021020"], bldNeon: ["#00e5ff","#ff2bd6","#19ff9b"],
                  sky: ["#020410","#050a22"], fog: "rgba(3,6,18,0.78)" },
    };
    const col = THEMES[this.track.theme] || THEMES.city;

    // ── горизонт / небо ──────────────────────────────────────────────────────
    {
      const skyG = ctx.createLinearGradient(0, 0, 0, horizY);
      skyG.addColorStop(0, col.sky[0]);
      skyG.addColorStop(1, col.sky[1]);
      ctx.fillStyle = skyG;
      ctx.fillRect(0, 0, CANVAS_W, horizY);
    }

    // ── ЗДАНИЯ (фон, за горизонтом) ─────────────────────────────────────────
    this._renderBuildings(ctx, col, horizY);

    // ── туман у горизонта ────────────────────────────────────────────────────
    {
      const fogG = ctx.createLinearGradient(0, horizY - 60, 0, horizY + 80);
      fogG.addColorStop(0, "rgba(0,0,0,0)");
      fogG.addColorStop(0.45, col.fog);
      fogG.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = fogG;
      ctx.fillRect(0, horizY - 60, CANVAS_W, 140);
    }
    ...
    for (let i = 0; i < steps; i++) {
      ...
      // ── обочина ─────────────────────────────────────────────────────────────
      const shoulderW0 = ROAD.shoulder * scale0;
      const shoulderW1 = ROAD.shoulder * scale1;
      ctx.fillStyle = i % 2 === 0 ? "rgba(7,9,16,0.92)" : "rgba(9,11,20,0.92)";
      ctx.beginPath();
      ctx.moveTo(centerX0 - roadW0 * 0.5 - shoulderW0, y0);
      ctx.lineTo(centerX0 - roadW0 * 0.5,              y0);
      ctx.lineTo(centerX1 - roadW1 * 0.5,              y1);
      ctx.lineTo(centerX1 - roadW1 * 0.5 - shoulderW1, y1);
      ctx.closePath(); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(centerX0 + roadW0 * 0.5,              y0);
      ctx.lineTo(centerX0 + roadW0 * 0.5 + shoulderW0, y0);
      ctx.lineTo(centerX1 + roadW1 * 0.5 + shoulderW1, y1);
      ctx.lineTo(centerX1 + roadW1 * 0.5,              y1);
      ctx.closePath(); ctx.fill();
      ...
      ctx.lineWidth   = Math.max(1.5, 3.5 * scale0);
      ctx.shadowColor = col.border;
      ctx.shadowBlur  = 6 * (1 - t0 * 0.7);
      ctx.beginPath();
      ctx.moveTo(centerX0 - roadW0 * 0.5, y0);
      ctx.lineTo(centerX1 - roadW1 * 0.5, y1);
      ctx.stroke();
      ...
      for (let l = 1; l < ROAD.lanes; l++) {
        ...
        if ((i + Math.floor(this.cameraZ / 55)) % 3 === 0) continue;
        ctx.strokeStyle = col.lane;
        ctx.lineWidth   = Math.max(1, 2.5 * scale0);
        ctx.beginPath();
        ctx.moveTo(lx0, y0); ctx.lineTo(lx1, y1);
        ctx.stroke();
      }
      ctx.restore();

      // ── фонарные столбы ──────────────────────────────────────────────────────
      if (i % 8 === 0 && t0 > 0.05 && t0 < 0.88) {
        const poleH  = 50 * scale0;
        const poleXL = centerX0 - roadW0 * 0.5 - 12 * scale0;
        const poleXR = centerX0 + roadW0 * 0.5 + 12 * scale0;
        ctx.save();
        ctx.strokeStyle = "rgba(70,80,110,0.5)";
        ctx.lineWidth = Math.max(1, 1.8 * scale0);
        ctx.beginPath(); ctx.moveTo(poleXL, y0); ctx.lineTo(poleXL, y0 - poleH); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(poleXR, y0); ctx.lineTo(poleXR, y0 - poleH); ctx.stroke();
        ctx.globalCompositeOperation = "lighter";
        const lampA = 0.38 * (1 - t0 * 0.5);
        ctx.fillStyle = `rgba(0,200,255,${lampA})`;
        ctx.beginPath(); ctx.arc(poleXL, y0 - poleH, 14 * scale0, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.arc(poleXR, y0 - poleH, 14 * scale0, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
      }
    ...
    if (this.mode === GAME_MODES.CAREER) {
      ...
      if (p0.visible) {
        ctx.save();
        ctx.globalCompositeOperation = "lighter";
        ctx.strokeStyle = "rgba(166,255,0,0.55)";
        ctx.lineWidth   = 5;
        ctx.shadowColor = "#a6ff00";
        ctx.shadowBlur  = 14;
        ctx.beginPath();
        ctx.moveTo(p0.x - 280 * p0.scale, p0.y);
        ctx.lineTo(p0.x + 280 * p0.scale, p0.y);
        ctx.stroke();
        ctx.restore();
      }
    }
  ...
  // ── здания на фоне ─────────────────────────────────────────────────────────
  _renderBuildings(ctx, col, horizY) {
    if (!this._bldSeed) this._bldSeed = this._genBuildings();
    const blds   = this._bldSeed;
    const scroll = (this.cameraZ * 0.022) % CANVAS_W;

    for (const b of blds) {
      const bx = ((b.x - scroll + CANVAS_W * 3) % (CANVAS_W * 2.2)) - b.w * 0.5;
      const by = horizY - b.h;
      ...
      for (let row = 0; row < b.winRows; row++) {
        for (let col2 = 0; col2 < b.winCols; col2++) {
          if (!b.winMask[row * b.winCols + col2]) continue;
          ctx.fillStyle = b.neon + "44";
          ctx.fillRect(bx + 4 + col2 * 9, by + 5 + row * 14, 5, 8);
        }
      }
      if (b.sign) {
        ...
        ctx.shadowBlur  = 7;
        ctx.strokeRect(bx + b.w * 0.15, by + b.h * 0.22, b.w * 0.7, b.h * 0.18);
      }
      ctx.restore();
    }
  }
  ...
  _genBuildings() {
    const neons  = ["#00e5ff","#ff2bd6","#a6ff00","#ffd000","#19ff9b","#ff6b00"];
    const colors = ["#060810","#07091a","#090c1e","#050710","#06090e","#0a0c18"];
    const blds   = [];
    let x = 0;
    while (x < CANVAS_W * 2.2) {
      const w    = 36 + Math.floor(Math.random() * 75);
      const h    = 55 + Math.floor(Math.random() * 175);
      const neon = neons[Math.floor(Math.random() * neons.length)];
      const wCols = Math.max(1, Math.floor(w / 9));
      const wRows = Math.max(1, Math.floor(h / 14));
      const mask  = new Uint8Array(wCols * wRows);
      for (let i = 0; i < mask.length; i++) mask[i] = Math.random() < 0.45 ? 1 : 0;
      blds.push({ x, w, h, color: colors[Math.floor(Math.random() * colors.length)], neon,
                  sign: Math.random() < 0.32, winCols: wCols, winRows: wRows, winMask: mask });
      x += w + 2 + Math.floor(Math.random() * 12);
    }
    return blds;
  }
  ...
  _renderMinimap() {
    ...
    ctx.lineWidth   = 2;
    roundRect(ctx, mx, my, w, h, 10);
    ctx.fill(); ctx.stroke();

    ctx.fillStyle = "rgba(0,229,255,0.05)";
    ctx.fillRect(mx + 6, my + 6, w - 12, h - 12);

    const pDz = this.player.z - this.cameraZ;
    if (pDz >= 0 && pDz <= 2400) {
      const t  = pDz / 2400;
      const py = my + h - 14 - t * (h - 28);
      const px = mx + w / 2 + (this.player.x / 500) * 38;
    ...
    for (const c of this.trafficPool) {
      ...
      const t  = dz / 2400;
      const ty = my + h - 14 - t * (h - 28);
      const tx = mx + w / 2 + (c.x / 500) * 38;
      ctx.fillRect(tx - 1.5, ty - 1.5, 3, 3);
    }

    if (this.mode === GAME_MODES.CAREER) {
      ctx.fillStyle = "rgba(166,255,0,0.82)";
      for (const r of this.rivals) {
        const dz = r.z - this.cameraZ;
        if (dz < 0 || dz > 2400) continue;
        const t  = dz / 2400;
        const ty = my + h - 14 - t * (h - 28);
        const tx = mx + w / 2 + (r.x / 500) * 38;
        ctx.fillRect(tx - 1.5, ty - 1.5, 3, 3);
      }
    }
